...
export default function FollowupSection({ latestConsultationId, patientId, existingFollowup }: Props) {
  ...
  if (followup?.responded_at && followup.status) {
    return (
      <div className={`rounded-xl px-5 py-4 mb-6 ${statusColor[followup.status]}`}>
        <p className="text-sm font-medium">Самочувствие: {statusLabel[followup.status]}</p>
        {followup.comment && (
  ...
  if (followup?.token) {
    ...
    return (
      <div className="bg-green-50 border border-green-200 rounded-xl px-5 py-4 mb-6">
        ...
        <p className="text-xs text-green-500 mt-2">Отправьте эту ссылку пациенту в мессенджере или по SMS</p>
      </div>
    )
  }
  ...
  return (
    <div className="mb-6">
      ...
      >
        {loading ? 'Создаю ссылку...' : 'Узнать самочувствие пациента'}
      </button>
    </div>
  )
}
