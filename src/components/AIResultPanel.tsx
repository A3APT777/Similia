...
export default function AIResultPanel({ aiResult, lang, onAssignRemedy, onClarify, clarifyingQuestions }: Props) {
  ...
  return (
    <div className="ai-glass ai-slide-up rounded-xl overflow-hidden">
      ...
      {/* Блок уточнений при low confidence */}
      {onClarify && (() => {
        const top = results[0]
        const second = results[1]
        if (!top) return null
        const isLow = top.confidence === 'low' || top.confidence === 'insufficient'
          ...
          <div className="mx-2 mb-2 p-3 rounded-xl bg-amber-50 border border-amber-200 space-y-2">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs font-semibold text-amber-700">
                {lang === 'ru' ? 'AI рекомендует уточнить' : 'AI recommends clarification'}
              </span>
            </div>
            <p className="text-[11px] text-amber-600">
              {lang === 'ru'
                ? 'Уверенность анализа низкая или разница между top-2 препаратами минимальна. Уточняющие вопросы помогут.'
                : 'Analysis confidence is low or the gap between top-2 remedies is minimal. Clarifying questions will help.'}
            </p>
            <button
            ...
            >
              {lang === 'ru' ? 'Получить уточняющие вопросы' : 'Get clarifying questions'}
            </button>
          </div>
        )
      })()}
