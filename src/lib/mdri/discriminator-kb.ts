...
export const DISCRIMINATOR_KB: DifferentialPairKB[] = [
  ...
  // ═══ 4. Bryonia vs Rhus-tox ═══
  {
    ...
    discriminators: [
      {
        id: 'BRY_RHUST_MORNING', type: 'modality',
        labelRu: 'Представьте: утро, пациент проснулся с болью. Что он делает?',
        labelInternal: 'morning motion response — binary discriminator',
        clinicalMeaning: 'Bryonia — не шевелится, любое движение усиливает, лежит как каменный. Rhus-t — скован, но встаёт и расхаживается, через 5-10 минут легче.',
        whyItMatters: 'ГЛАВНОЕ противопоставление в гомеопатии. Бинарный выбор, нет промежутка. Один лежит — другой ходит.',
        evidenceLevel: 'high', falsePositiveRisk: 'low',
        options: [
          { id: 'lies_still', labelRu: 'Лежит неподвижно — боится пошевелиться, каждое движение усиливает боль',
            effect: 'supports_a', confidenceWeight: 1.0,
            mappedSymptoms: [
              { canonicalKey: 'MOTION_AGG', rubric: 'worse motion any lies still', weight: 3, category: 'general', modality: { pairId: 'motion_rest', value: 'agg' } },
              { canonicalKey: 'REST_AMEL', rubric: 'better rest lying still', weight: 2, category: 'general' },
            ] },
          { id: 'gets_up_walks', labelRu: 'Встаёт и расхаживается — первые шаги тяжело, но через несколько минут легче',
            effect: 'supports_b', confidenceWeight: 1.0,
            mappedSymptoms: [
              { canonicalKey: 'FIRST_MOTION_AGG', rubric: 'stiffness joints worse first motion better continued', weight: 3, category: 'general', modality: { pairId: 'motion_rest', value: 'amel' } },
              { canonicalKey: 'RESTLESS', rubric: 'restlessness cannot lie still must move', weight: 2, category: 'mental' },
            ] },
          { id: 'motion_na', labelRu: 'Ситуация не подходит — боли нет утром или движение не связано с болью',
