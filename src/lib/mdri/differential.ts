...
export function buildDifferentialPrompt(ctx: DifferentialContext): string {
  ...
  return `Ты — гомеопат-дифференциатор. Твоя задача — НЕ подтвердить выбор, а РАЗЛИЧИТЬ препараты.
...
${conflictStr}${altStrStr}

ЗАДАЧА:
Сгенерируй 3-5 вопросов которые РАЗЛИЧАТ эти препараты.

ПРАВИЛА:
- Каждый вопрос должен ДИФФЕРЕНЦИРОВАТЬ: ответ "да" усиливает один препарат и ослабляет другой
- НЕ задавай общие вопросы ("как вы себя чувствуете?")
- Вопросы должны быть про КОНКРЕТНЫЕ клинические признаки
- Фокус на линзах с наибольшей разницей между препаратами
- Вопросы на русском языке, понятные врачу-гомеопату
