...
6. ЭТИОЛОГИЯ (Органон §5) — ВЫСШИЙ ПРИОРИТЕТ. "После горя", "с тех пор как развелась", "после прививки" → ВСЕГДА отдельный symptom с weight: 3. Этиология ВАЖНЕЕ любого отдельного симптома.
7. Если симптом содержит модальность ("хуже утром", "лучше от движения") — добавь И в symptoms, И в modalities.
8. Сторонность ("справа", "слева", "левая сторона") → добавь в rubric как "right side" / "left side", weight: 2.
9. Ощущения "как будто", "ощущение что" → peculiar, weight: 3. Примеры: "как будто опускается" → bearing down sensation, "как молотки" → headache hammering.
...
## NEVER WELL SINCE ("с тех пор как")
Паттерн: "с тех пор как X — появилось Y". Это этиология + симптом. Извлекай ОБА:
- "С развода головные боли" → ailments from grief divorce (w3) + headache chronic since grief (w2)
- "После прививки экзема" → ailments from vaccination (w3) + eczema since vaccination (w2)
- "После испуга заикается" → ailments from fright (w3) + stammering since fright (w2)

## CONCOMITANTS (сопутствующие)
Если симптомы связаны ("с", "одновременно", "при этом") — объединяй в одну rubric:
- "Головная боль с тошнотой" → headache WITH nausea (w2, не два отдельных w1)
- "Рвота и понос одновременно" → vomiting WITH diarrhea simultaneous (w3)
- "Кашель с рвотой" → cough WITH vomiting (w2)

## ВЫДЕЛЕНИЯ (характер)
Цвет, запах, консистенция выделений — peculiar keynotes:
- "Жёлтые густые" → discharge yellow thick bland (Puls keynote, w2)
- "Зелёные корки" → discharge green crusty (w2)
- "Кислый запах" → perspiration sour odor (w2)
- "Зловонные" → discharge offensive (w2)

## ПОЗИЦИЯ СНА
Как спит — important general:
- "на животе" → sleep position abdomen (w3 — peculiar)
- "на спине раскинув руки" → sleep position back arms spread (w2)
- "калачиком" → sleep position curled up fetal (w2)
...
Извлекай если текст указывает "хуже от X", "лучше от X", или КОСВЕННО через симптомы:
- heat_cold: "зябкий/мёрзнет/кутается" → amel, "жаркий/тепло хуже/духота" → agg
- motion_rest: "движение хуже/лежит неподвижно" → agg, "движение лучше/не может сидеть/бегает" → amel
- open_air: "свежий воздух лучше/открывает окно" → amel, "сквозняк хуже" → agg
- consolation: "утешение хуже" → agg, "утешение помогает/ищет сочувствие" → amel
- company_alone: "хочет быть один/уходит" → agg, "лучше в компании/боится одиночества" → amel
- pressure: "давление лучше/прижимает руку" → amel, "прикосновение хуже" → agg
- morning_evening: "утром хуже/еле встаёт утром" → agg, "вечером хуже" → agg
- sea: "на море лучше" → amel
- eating: "после еды хуже/вздутие после еды" → agg
- menses: "во время месячных хуже" → agg

ВАЖНО: если в тексте "зябкий" → это И symptom chilly (general), И модальность heat_cold: amel.
Если "голова хуже утром" → это И symptom headache worse morning, И модальность morning_evening: agg.
Не пропускай модальности из контекста симптомов!

## СЕМЕЙНЫЙ АНАМНЕЗ

Извлекай если текст упоминает болезни родственников (мать, отец, бабушка, дедушка, в семье):
tuberculosis, cancer, diabetes, psoriasis, asthma, alcoholism, heart disease, eczema, warts, syphilis, gonorrhea, allergies, depression

Также извлекай: "мать — мигрени" → migraine, "дед — рак" → cancer, "в семье аллергики" → allergies
