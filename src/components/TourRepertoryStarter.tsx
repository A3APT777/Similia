...
import { startRepertoryTour, destroyActiveTour } from '@/lib/tour'
import { t } from '@/lib/i18n'
import { useLanguage } from '@/hooks/useLanguage'
import 'driver.js/dist/driver.css'

export default function TourRepertoryStarter() {
  const { lang } = useLanguage()
  const [tourActive, setTourActive] = useState(false)

  useEffect(() => {
    if (localStorage.getItem('tour_repertory_active') !== 'true') return
    setTourActive(true)
    const timer = setTimeout(() => {
      startRepertoryTour(lang)
    }, 800)
