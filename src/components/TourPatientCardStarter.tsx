...
import { startPatientCardTour, destroyActiveTour } from '@/lib/tour'
import { t } from '@/lib/i18n'
import { useLanguage } from '@/hooks/useLanguage'
import 'driver.js/dist/driver.css'

export default function TourPatientCardStarter() {
  const { lang } = useLanguage()
  const [tourActive, setTourActive] = useState(false)

  useEffect(() => {
    if (localStorage.getItem('tour_patient_active') !== 'true') return
    setTourActive(true)
    const timer = setTimeout(() => {
      startPatientCardTour(lang)
    }, 600)
