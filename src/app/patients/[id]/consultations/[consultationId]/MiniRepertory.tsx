...
import MiniRepertoryTutorial from './MiniRepertoryTutorial'
    ...
    <div className="flex flex-col h-full" style={{ backgroundColor: 'var(--sim-bg)' }}>
          ...
          <div className="flex rounded-md overflow-hidden border" style={{ borderColor: '#c8b89a' }}>
            {(['ru', 'en'] as const).map(l => (
              <button
                ...
                style={{
                  backgroundColor: repoLang === l ? '#2d6a4f' : 'transparent',
                  color: repoLang === l ? '#fff' : '#9a8a6a',
                }}
              >
                {l.toUpperCase()}
              </button>
      ...
      {tutStep < 0 && (
        <div className="px-3">
          <FirstTimeHint id="mini_repertory">
            {lang === 'ru'
              ? 'Это мини-реперторий Кента — 74 482 рубрики. Введите симптом, добавьте в анализ кнопкой [+], назначьте препарат. Кнопка «📚 Обучение» в тулбаре — пошаговый тур из 11 шагов.'
              : "This is Kent's mini-repertory — 74,482 rubrics. Type a symptom, add to analysis with [+], prescribe. '📚 Tutorial' button — 11-step tour."}
          </FirstTimeHint>
        </div>
      )}
      ...
      <div data-tour="mini-results" className={`flex-1 overflow-y-auto${tutStep === 2 ? ' mini-tut-glow' : ''}`}>
        {loading ? (
          ...
          <div className="flex items-center justify-center h-16 text-[12px]" style={{ color: searchError ? '#dc2626' : '#9a8a6a' }}>
            {searchError
                    ...
                    <div className="px-4 pb-3 pt-1" style={{ backgroundColor: 'rgba(45,106,79,0.03)', borderTop: '1px solid #ede7dd' }}>
                      <div className="flex flex-wrap gap-1 mb-2">
                        {r.remedies.map((rem, i) => (
                          ...
                          >
                            <span style={{ color: rem.grade >= 3 ? '#2d6a4f' : rem.grade === 2 ? '#1c1a14' : '#9a9a8a', fontWeight: rem.grade >= 3 ? 700 : rem.grade === 2 ? 500 : 400, fontStyle: rem.grade === 2 ? 'italic' : 'normal', fontSize: rem.grade >= 3 ? '12px' : '11px' }}>
                              {rem.abbrev}
                            </span>
                          </button>
                        ))}
      ...
      {/* Туториал */}
      {tutStep >= 0 && (
        <MiniRepertoryTutorial
          step={tutStep}
          lang={lang}
          addedCount={tutAddedCount}
          onNext={handleTutNext}
          onExit={handleTutExit}
        />
      )}
